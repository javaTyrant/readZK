...
/**
 * 这个类维护了树形结构.这个类没有任何的网络连接,或者被客户端连接,所以可以被单独的测试.
 * 这个数维护了两种数据结构:
 * 1.一个hashtable,映射了一个完整的路径到datanodes
 * 2.一个树形的dataNodes.
 * 所有对路径的访问都通过这个hashtable.这个树只有在被序列化到硬盘的时候才会被遍历.
